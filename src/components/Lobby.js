...
<h1 style={styles.title}>RAMINO<br />POKERATO</h1>
<p style={styles.subtitle}>LA VARIANTE PAZZA</p>
</div>

```
  {!mode ? (
    <div style={styles.btnGroup}>
      <button onClick={() => setMode("create")} style={styles.btnPrimary}>
        CREA PARTITA
      </button>
    ...
    <div style={styles.card}>
      ...
      {mode === "join" && (
        <div>
          <label style={{ ...styles.label, marginTop: 16 }}>CODICE STANZA</label>
          <input
            value={roomCode}
...
<div style={styles.hero}>
<div style={styles.cardIcon}>🃏</div>
<h2 style={{ …styles.title, fontSize: 28 }}>SALA D’ATTESA</h2>
</div>

```
  <div style={styles.roomCodeBox}>
    <div style={styles.roomCodeLabel}>CODICE STANZA</div>
    <div style={styles.roomCode}>{roomCode}</div>
    <div style={styles.roomCodeHint}>Condividi con i tuoi amici</div>
  </div>

  <div style={styles.card}>
    <div style={styles.label}>GIOCATORI ({playerList.length}/6)</div>
    {playerList.map((p, i) => (
      <div key={p.id} style={styles.playerRow}>
        <div style={{ ...styles.playerDot, background: COLORS[i] }} />
        <span style={styles.playerName}>{p.name}</span>
        {p.id === playerId && <span style={styles.youBadge}>TU</span>}
      </div>
    ))}
  </div>
  ...
  {isHost ? (
    <button
      ...
      style={playerList.length >= 2 ? styles.btnPrimary : styles.btnDisabled}
    >
      {playerList.length < 2 ? "ASPETTA ALTRI GIOCATORI..." : "INIZIA PARTITA"}
    </button>
  ) : (
    <p style={styles.waitText}>In attesa che l'host inizi...</p>
  )}
</div>
```
...
cursor: “pointer”,
letterSpacing: 2,
boxShadow: “0 4px 20px rgba(240,192,64,0.35), 0 2px 4px rgba(0,0,0,0.3)”,
fontFamily: “‘Georgia’, serif”,
