...
const logs = room.log || [];
await update(ref(db, `rooms/${roomCode}`), { log: […logs.slice(-30), msg] });
};
...
if (!aperturaId) { showMsg("Combinazione non valida per aprire! (niente jolly)"); return; }
if (me.apertureUsate?.[aperturaId]) {
  showMsg(`Hai gia usato: ${APERTURE_TYPES.find(a => a.id === aperturaId)?.label}`);
  return;
}
...
setSelected([]);
showMsg(`Aperto con ${APERTURE_TYPES.find(a => a.id === aperturaId)?.label}!`, "success");
...
if (!isMyTurn) { showMsg(“Non e il tuo turno!”); return; }
if (!room.drawnThisTurn) { showMsg(“Devi prima pescare!”); return; }
if (me.apertureUsate?.chiusura) { showMsg(“Chiusura gia usata!”); return; }
...
for (const pid of playerOrder) {
  scores[pid] = pid === playerId ? 0 : handPoints(room.hands?.[pid] || []);
}
...
if (!isValidCombination(newCards)) {
  showMsg("Le carte non formano una combinazione valida!"); return;
}
...
const newTable = room.table.map(c =>
  c.id === comboId ? { ...c, cards: newComboCards } : c
);
...
// Touch drag
const touchDrag = useRef({ active: false, startIdx: null, startX: 0 });

const sortedPlayers = playerOrder.map((pid, i) => ({
…players[pid],
id: pid,
color: COLORS[i],
handCount: (room.hands?.[pid] || []).length,
}));
...
{showTable ? “NASCONDI TAVOLO” : “MOSTRA TAVOLO”}
</button>
...
```
  ...
  <div style={{
    ...
    <span style={{ color: isMyTurn ? "#f0c040" : "#4a6a7a", fontWeight: 800, letterSpacing: 2 }}>
      {isMyTurn ? "TOCCA A TE!" : `TURNO DI ${players[currentPid]?.name?.toUpperCase() || "..."}`}
    </span>
  </div>
  ...
  <div style={s.playersRow}>
    {sortedPlayers.map(p => (
      <div key={p.id} style={{
        ...
        <div style={{ ...s.playerColorDot, background: p.color }} />
        <div>
  ...
  {showAperture && (
    <div style={s.aperturePanel}>
      <div style={s.aperturePanelTitle}>APERTURE</div>
      {sortedPlayers.map(p => (
        <div key={p.id} style={s.aperturePlayerRow}>
          ...
          <div style={s.apertureBadges}>
            {APERTURE_TYPES.map(a => {
              const used = !!p.apertureUsate?.[a.id];
              return (
                <div key={a.id} style={{
                  ...s.apertureBadge,
                  background: used ? "rgba(255,255,255,0.04)" : `${p.color}18`,
                  color: used ? "#3a4a5a" : p.color,
                  border: used ? "1px solid rgba(255,255,255,0.04)" : `1px solid ${p.color}44`,
                  textDecoration: used ? "line-through" : "none",
                }}>
                  {used && <span style={{ marginRight: 2, color: "#e74c3c" }}>✗</span>}
                  {a.label}
                </div>
              );
            })}
  ...
  {showTable && (
    <div style={s.table}>
      <div style={s.tableTitle}>TAVOLO</div>
      {(!room.table || room.table.length === 0) ? (
        <div style={{ color: "#2a4a5a", fontSize: 12, padding: "8px 0" }}>Nessuna combinazione ancora</div>
      ) : (
        <div style={s.tableCombos}>
          {room.table.map(combo => (
            <div key={combo.id} style={{
              ...s.tableCombo,
              borderColor: `${combo.color}44`,
            }}>
              <div style={{ color: combo.color, fontSize: 9, fontWeight: 800, letterSpacing: 1, marginBottom: 6 }}>
                {combo.playerName?.toUpperCase()} — {APERTURE_TYPES.find(a => a.id === combo.type)?.label}
              </div>
              <div style={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
                {combo.cards.map((card, idx) => (
                  <div
                    key={card.id}
                    onClick={() => card.isJoker ? swapJoker(combo.id, idx) : null}
      ...
      {/* DISCARD PILE */}
      <div style={s.discardArea}>
        <div style={s.tableTitle}>SCARTI</div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {room.topDiscard ? (
            <div onClick={drawFromDiscard} style={{ cursor: isMyTurn && !room.drawnThisTurn ? "pointer" : "default" }}>
              <Card card={room.topDiscard} />
            </div>
          ) : (
            <div style={s.emptyDiscard}>VUOTO</div>
          )}
          <div style={{ color: "#2a4a5a", fontSize: 11 }}>
            {isMyTurn && !room.drawnThisTurn && room.topDiscard && "Tocca per prendere"}
          </div>
        </div>
      </div>
    </div>
  ...
  <div style={s.handArea}>
    ...
    {/* Auto apertura hint */}
    {selected.length > 0 && detectedApertura && !me.apertureUsate?.[detectedApertura] && (
      <div style={s.aperturaHint}>
        APERTURA RILEVATA: {APERTURE_TYPES.find(a => a.id === detectedApertura)?.label}
      </div>
    )}

    {/* Cards - overlapping style */}
        ...
        return (
          <div
            ...
            style={{
              ...
              zIndex: isSelected ? 100 : isDragOver ? 90 : idx,
              position: "relative",
              transition: "margin 0.1s, z-index 0.1s",
      ...
      {APERTURE_TYPES.map(a => {
        ...
        return (
          <div key={a.id} style={{
            ...
            border: used ? "1px solid rgba(255,255,255,0.04)" : `1px solid ${myColor}44`,
            textDecoration: used ? "line-through" : "none",
            fontSize: 9,
          }}>
            {used && <span style={{ color: "#e74c3c", marginRight: 2 }}>✗</span>}
            {a.label}
          </div>
        );
      })}
  ...
  {isMyTurn && (
    <div style={s.actions}>
      {!room.drawnThisTurn ? (
        <button onClick={drawFromDeck} style={s.actionBtn("#3498db")}>
          PESCA
        </button>
      ) : (
        <>
          {selected.length > 0 && detectedApertura && !me.apertureUsate?.[detectedApertura] && (
            <button onClick={handleApertura} style={s.actionBtn("#9b59b6")}>
              APRI: {APERTURE_TYPES.find(a => a.id === detectedApertura)?.label}
            </button>
          )}
          ...
          <button onClick={discardCard} style={s.actionBtn("#e74c3c")}>
            SCARTA
          </button>
        </>
      )}
  ...
  <div ref={logRef} style={s.log}>
    {(room.log || []).map((l, i) => (
      <span key={i} style={s.logItem}>{l} · </span>
    ))}
  </div>
      ...
      <div style={s.modal}>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          ...
          <h3 style={s.modalTitle}>
            {players[room.handWinner]?.name?.toUpperCase()} CHIUDE!
          </h3>
          <p style={{ color: "#4a6a7a", fontSize: 12, letterSpacing: 2 }}>MANO {room.mano}</p>
        </div>
        {sortedPlayers.map(p => (
          <div key={p.id} style={s.scoreRow}>
            <span style={{ color: p.color, fontWeight: 800 }}>{p.name?.toUpperCase()}</span>
            <span>
        ...
        {room.host === playerId && (
          <button onClick={startNewHand} style={{ ...s.actionBtn("#f0c040", "#061a26"), width: "100%", marginTop: 24 }}>
            PROSSIMA MANO
          </button>
        )}
        {room.host !== playerId && (
...
background: “#061a26”, color: “#f0c040”, fontSize: 18,
...
headerTitle: {
color: “#f0c040”, fontWeight: 900, fontSize: 14, letterSpacing: 3,
},
headerMano: {
color: “#2a4a5a”, fontSize: 11, marginLeft: 10, letterSpacing: 2,
},
headerBtn: {
background: “rgba(255,255,255,0.05)”,
border: “1px solid rgba(255,255,255,0.08)”,
color: “#4a8fa6”, borderRadius: 6,
...
aperturePanelTitle: {
color: “#4a8fa6”, fontSize: 10, letterSpacing: 3,
...
tableTitle: {
color: “#4a8fa6”, fontSize: 9, letterSpacing: 3,
fontWeight: 800, marginBottom: 8,
},
tableCombos: {
...
color: “#f0c040”, fontSize: 9, cursor: “pointer”,
...
handArea: {
flex: 1,
background: “linear-gradient(180deg, #061a26 0%, #051520 100%)”,
padding: “12px 12px 0”,
...
color: “#9b59b6”, fontSize: 10, letterSpacing: 2,
fontWeight: 800, marginBottom: 8,
},
handCards: {
display: “flex”,
...
color: “#2a4a5a”, fontSize: 10, whiteSpace: “nowrap”,
overflowX: “auto”,
},
logItem: { marginRight: 4 },
modalOverlay: {
